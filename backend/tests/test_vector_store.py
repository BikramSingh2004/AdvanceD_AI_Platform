        ...
        with patch("faiss.IndexFlatL2") as mock_index_class:
            with patch("os.path.exists", return_value=False):
                with patch("os.makedirs"):
                    with patch("app.services.vector_store.get_embedding_model") as mock_model:
            ...
            with patch.object(vs, "_document_store", {}):
                with patch.object(vs, "_chunk_to_doc", {}):
                    with patch.object(vs, "_current_idx", 0):
                        with patch("app.services.vector_store.get_embedding_model") as mock_model:
                            mock_model.return_value.encode.return_value = np.zeros((3, 384), dtype=np.float32)
                            
                            with patch("app.services.vector_store.chunk_text") as mock_chunk:
                                ...
                                with patch("app.services.vector_store._save_sync"):
                                    with patch("app.services.vector_store._remove_document_sync"):
                                        result = vs._index_document_sync("doc-123", "Test content", None)
